    ...
    <>
      ...
      {showToolbar && (
        ...
        >
          ...
          >
            ...
            {/* Change Angle */}
            <Tooltip content="Change Viewing Angle">
              <Button
                variant="soft"
                color="blue"
                size="3"
                onClick={async (e) => {
                  ...
                  const reader = new FileReader();
                  reader.onload = (ev) => {
                    setAngleImageDataUrl(ev.target?.result as string);
      ...
      <AngleChangeModal
        ...
        onClose={() => {
          ...
          setAngleImageDataUrl(null);
        }}
        imageDataUrl={angleImageDataUrl}
        onApply={(newImageDataUrl: string) => {
          const currentFrame = editor.getShape(shapeId);
          if (!currentFrame) return;
          ...
          img.onload = () => {
            ...
            // Create new frame to the right (same pattern as handleGenerate)
            const newFrameId = createShapeId();
            const gap = 2000;
            const branchCount = getExistingBranchCount(shapeId);
            const verticalGap = frameH + 200;
            ...
            // Create arrow connecting source frame to new frame
            const arrowId = createShapeId();
            editor.createShapes([
              {
                id: arrowId,
            ...
            toast.success("Angle changed — new frame created!");
          };
